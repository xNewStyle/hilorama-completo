#!/usr/bin/env python3
"""
Tester conversacional humano V36 para WhatsApp IA Hilorama.

Este tester NO reemplaza al tester masivo V35. Lo complementa.
V35 prueba muchas preguntas/pedidos rápidos.
V36 prueba conversaciones más reales, raras y largas:
...
Uso recomendado:
  python tools/whatsapp_ia_human_tester_v36.py --base-url https://hilorama-celular.onrender.com --pin TU_PIN --limit 50
...
def run_case(case: Case, base_url: str, pin: str, timeout: float, sleep_s: float, abort_if_not_tester: bool) -> Dict[str, Any]:
    ...
    conversacion_id = f"TEST-V36-{case.case_id}"
    ...
    for idx, turn in enumerate(case.turns):
        payload = {
            ...
            "telefono": f"TEST-V36-{case.case_id}",
...
def write_reports(results: List[Dict[str, Any]], out_dir: Path, inventory: Dict[str, Any]):
    ...
    json_path = out_dir / f"wa_human_test_results_v36_{stamp}.json"
    fail_csv_path = out_dir / f"wa_human_test_failures_v36_{stamp}.csv"
    conv_csv_path = out_dir / f"wa_human_test_conversaciones_v36_{stamp}.csv"
    html_path = out_dir / f"wa_human_test_report_v36_{stamp}.html"
    ...
    html_doc = f"""<!doctype html><meta charset='utf-8'><title>WA IA Tester Humano V36</title>
    ...
    <h1>Reporte tester conversacional humano WhatsApp IA Hilorama V36</h1>
