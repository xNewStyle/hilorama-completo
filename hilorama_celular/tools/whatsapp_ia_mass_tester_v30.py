...
Uso recomendado:
  ...
  python tools/whatsapp_ia_mass_tester_v30.py --base-url https://hilorama-celular.onrender.com --pin TU_PIN --limit 1000 --sleep 0.2
...
IMPORTANTE: requiere V30 en el servidor porque usa tester_mode/dry_run para NO guardar conversaciones reales.
"""
...
def grade_case(case: Case, final_resp: Dict[str, Any], all_resps: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
    ...
    for phrase in exp.get("must_contain_any", []) or []:
        if not any(text_norm(part) in tn for part in ([phrase] if isinstance(phrase, str) else phrase)):
            reasons.append(f"no contiene esperado: {phrase}")
...
def write_reports(results: List[Dict[str, Any]], out_dir: Path):
    ...
    <h1>Reporte tester WhatsApp IA Hilorama</h1>
