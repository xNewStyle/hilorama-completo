#!/usr/bin/env python3
"""
Tester conversacional humano V52 para WhatsApp IA Hilorama.

Este tester NO reemplaza al tester masivo V35. Lo complementa.
V35 prueba muchas preguntas/pedidos rápidos.
V52 prueba conversaciones mucho más difíciles y humanas:
...
Uso recomendado:
  python tools/whatsapp_ia_human_tester_v52.py --base-url https://hilorama-celular.onrender.com --pin TU_PIN --limit 50
  python tools/whatsapp_ia_human_tester_v52.py --base-url https://hilorama-celular.onrender.com --pin TU_PIN --limit 200 --sleep 0.2
...
FORBIDDEN_PUBLIC_PHRASES = [
    ...
    # V52: señales de productos inventados por abrir conversación o corregir contexto.
    "Pregunta x1", "Pedio Kotton Milk x1", "Velluto Perdon Todo Eso Seria x1", "Si x5", "Si x1", "Todos X2 x1", "Antes Pedir x60", "Es 429 x1", "Dices 429 x1", "Velluto Tendras x1", "Velluto Manejaz x1", "Velluto Dos x429",
]
...
def generate_cases(limit: int, inventory: Dict[str, Any], seed: int = 4848) -> List[Case]:
    ...
    # V52: casos difíciles externos listos para copiar/pegar y estresar la IA.
    # Están en tools/casos_tester_v52_envios_conversacion_hilorama.json. Si no existe el archivo, el tester sigue con los casos internos.
    extra_cases_path = Path(__file__).with_name("casos_tester_v52_envios_conversacion_hilorama.json")
    if extra_cases_path.exists():
        try:
            ...
            for obj in extra_data:
                ...
                if turns:
                    add(obj.get("category") or "v40_extra", turns, obj.get("expect") or {})
        except Exception as exc:
            print(f"Aviso: no pude cargar casos V52 externos: {exc}", file=sys.stderr)
...
def buffer_wait_seconds(resp: Dict[str, Any]) -> int:
    """Devuelve segundos de espera del buffer si el backend los reporta."""
    candidates = [
        _get_nested(resp, ["v30", "buffer", "espera_segundos"], None),
        _get_nested(resp, ["buffer", "espera_segundos"], None),
        _get_nested(resp, ["cierre_diferido", "espera_segundos"], None),
    ]
...
def grade_case(case: Case, responses: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
    ...
    if case.expect.get("expected_no_pedidos"):
        all_tmp: List[Dict[str, Any]] = []
        for r in responses:
            all_tmp.extend(collect_items(r))
        if all_tmp:
            reasons.append("no debía detectar/agregar pedidos en este caso")

    if case.expect.get("should_wait_for_more"):
        first_wait = buffer_wait_seconds(responses[0]) if responses else 0
        # Este tester no puede medir tiempo real de WhatsApp, pero sí verifica que el backend reporte ventana de espera.
        if first_wait < 20:
            reasons.append(f"no dio suficiente ventana de espera para saludo/mensaje incompleto: {first_wait}s")
    ...
    # V52: algunos casos deben contener varias piezas a la vez (ej. ojo + seguridad),
...
def run_case(case: Case, base_url: str, pin: str, timeout: float, sleep_s: float, abort_if_not_tester: bool) -> Dict[str, Any]:
    ...
    conversacion_id = f"TEST-V52-{case.case_id}"
    ...
    for idx, turn in enumerate(case.turns):
        payload = {
            ...
            "telefono": f"TEST-V52-{case.case_id}",
...
def write_reports(results: List[Dict[str, Any]], out_dir: Path, inventory: Dict[str, Any]):
    ...
    json_path = out_dir / f"wa_human_test_results_v52_{stamp}.json"
    fail_csv_path = out_dir / f"wa_human_test_failures_v52_{stamp}.csv"
    conv_csv_path = out_dir / f"wa_human_test_conversaciones_v52_{stamp}.csv"
    html_path = out_dir / f"wa_human_test_report_v52_{stamp}.html"
    ...
    html_doc = f"""<!doctype html><meta charset='utf-8'><title>WA IA Tester Humano V52</title>
    ...
    <h1>Reporte tester conversacional humano WhatsApp IA Hilorama V52</h1>
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=1000, help="Número de conversaciones humanas a generar/probar")
