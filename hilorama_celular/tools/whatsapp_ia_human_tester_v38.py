#!/usr/bin/env python3
"""
Tester conversacional humano V38 para WhatsApp IA Hilorama.

Este tester NO reemplaza al tester masivo V35. Lo complementa.
V35 prueba muchas preguntas/pedidos rápidos.
V38 prueba conversaciones más reales, raras y largas:
...
Uso recomendado:
  python tools/whatsapp_ia_human_tester_v38.py --base-url https://hilorama-celular.onrender.com --pin TU_PIN --limit 50
  python tools/whatsapp_ia_human_tester_v38.py --base-url https://hilorama-celular.onrender.com --pin TU_PIN --limit 200 --sleep 0.2
...
FORBIDDEN_PUBLIC_PHRASES = [
    ...
    # V38: señales de productos inventados por abrir conversación o corregir contexto.
...
def generate_cases(limit: int, inventory: Dict[str, Any], seed: int = 3636) -> List[Case]:
    ...
    # V38: casos complejos externos listos para copiar/pegar y estresar la IA.
    # Están en tools/casos_tester_v38_complejos_hilorama.json. Si no existe el archivo, el tester sigue con los casos internos.
    extra_cases_path = Path(__file__).with_name("casos_tester_v38_complejos_hilorama.json")
    if extra_cases_path.exists():
        try:
            ...
            for obj in extra_data:
                ...
                if turns:
                    add(obj.get("category") or "v38_extra", turns, obj.get("expect") or {})
        except Exception as exc:
            print(f"Aviso: no pude cargar casos V38 externos: {exc}", file=sys.stderr)
...
def run_case(case: Case, base_url: str, pin: str, timeout: float, sleep_s: float, abort_if_not_tester: bool) -> Dict[str, Any]:
    ...
    conversacion_id = f"TEST-V38-{case.case_id}"
    ...
    for idx, turn in enumerate(case.turns):
        payload = {
            ...
            "telefono": f"TEST-V38-{case.case_id}",
...
def write_reports(results: List[Dict[str, Any]], out_dir: Path, inventory: Dict[str, Any]):
    ...
    json_path = out_dir / f"wa_human_test_results_v38_{stamp}.json"
    fail_csv_path = out_dir / f"wa_human_test_failures_v38_{stamp}.csv"
    conv_csv_path = out_dir / f"wa_human_test_conversaciones_v38_{stamp}.csv"
    html_path = out_dir / f"wa_human_test_report_v38_{stamp}.html"
    ...
    html_doc = f"""<!doctype html><meta charset='utf-8'><title>WA IA Tester Humano V38</title>
    ...
    <h1>Reporte tester conversacional humano WhatsApp IA Hilorama V38</h1>
