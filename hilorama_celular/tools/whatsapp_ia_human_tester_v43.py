#!/usr/bin/env python3
"""
Tester conversacional humano V43 para WhatsApp IA Hilorama.

Este tester NO reemplaza al tester masivo V35. Lo complementa.
V35 prueba muchas preguntas/pedidos rápidos.
V43 prueba conversaciones mucho más difíciles y humanas:
...
Uso recomendado:
  python tools/whatsapp_ia_human_tester_v43.py --base-url https://hilorama-celular.onrender.com --pin TU_PIN --limit 50
  python tools/whatsapp_ia_human_tester_v43.py --base-url https://hilorama-celular.onrender.com --pin TU_PIN --limit 200 --sleep 0.2
...
FORBIDDEN_PUBLIC_PHRASES = [
    ...
    # V43: señales de productos inventados por abrir conversación o corregir contexto.
...
def generate_cases(limit: int, inventory: Dict[str, Any], seed: int = 3636) -> List[Case]:
    ...
    # V43: casos difíciles externos listos para copiar/pegar y estresar la IA.
    # Están en tools/casos_tester_v43_dificiles_hilorama.json. Si no existe el archivo, el tester sigue con los casos internos.
    extra_cases_path = Path(__file__).with_name("casos_tester_v43_dificiles_hilorama.json")
    if extra_cases_path.exists():
        try:
            ...
            print(f"Aviso: no pude cargar casos V43 externos: {exc}", file=sys.stderr)
...
def grade_case(case: Case, responses: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
    ...
    # V43: algunos casos deben contener varias piezas a la vez (ej. ojo + seguridad),
...
def run_case(case: Case, base_url: str, pin: str, timeout: float, sleep_s: float, abort_if_not_tester: bool) -> Dict[str, Any]:
    ...
    conversacion_id = f"TEST-V43-{case.case_id}"
    ...
    for idx, turn in enumerate(case.turns):
        payload = {
            ...
            "telefono": f"TEST-V43-{case.case_id}",
...
def write_reports(results: List[Dict[str, Any]], out_dir: Path, inventory: Dict[str, Any]):
    ...
    json_path = out_dir / f"wa_human_test_results_v43_{stamp}.json"
    fail_csv_path = out_dir / f"wa_human_test_failures_v43_{stamp}.csv"
    conv_csv_path = out_dir / f"wa_human_test_conversaciones_v43_{stamp}.csv"
    html_path = out_dir / f"wa_human_test_report_v43_{stamp}.html"
    ...
    html_doc = f"""<!doctype html><meta charset='utf-8'><title>WA IA Tester Humano V43</title>
    ...
    <h1>Reporte tester conversacional humano WhatsApp IA Hilorama V43</h1>
